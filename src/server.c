...
// Server functions
static int set_socket_addr(char *ip, int port, struct sockaddr_in *server_addr) 
{
    // sets server_addr
...
int broadcast_out_join(int broadcasting_socket)
{
...
int broadcast_out_approve(const int out_socket, const char *node_id)
{
    char *id = server_node.id;
    char *name = server_node.name;
...
int broadcast_out_disapprove(const int out_socket, const char *node_ip, int node_port, const char *disapprove_reason)
{
...
static int message_interchange_out(int socket, char *message, size_t len, int port)
{
    ...
    switch(type)
    {
        case MSG_TYPE_NETCONN:
            broadcast_out_join(socket);
            break;
        case MSG_TYPE_NETCONN_APPROVED:
            broadcast_out_approve(socket, id);
            break;
        case MSG_TYPE_NETCONN_DISAPPROVED:
            char *node_ip = strtok(data, DATA_DELIMITER);
            char *node_port = strtok(NULL, DATA_DELIMITER);
            char *disapprove_reason = strtok(NULL, DATA_DELIMITER);
            broadcast_out_disapprove(socket, node_ip, node_port, disapprove_reason);
            break;
...
int run_server(const char *name, const char *ip, const int port, const int backlog)
{
    // setup server address and sockets
    struct sockaddr_in server_addr;
    if (set_socket_addr(ip, port, &server_addr) < 0)
    {
        handle_errors("run_server - Error setting server address", strerror(errno));
        return -1;
    }
    ...
    if (listen_socket < 0)
    {
        handle_errors("run_server - Error creating listen_socket", strerror(errno));
        return -2;
    }
    ...
    while (1)
    {
        ...
        if (ready_amount == -1)
        {
            handle_errors("run_server - Error in select", strerror(errno));
            return -4;
        }
        ...
        for (int i = 3; i < max_fd + 1; i++) // i = 3 because 0, 1, 2 are reserved to stdin, stdout, stderr resepctively
        {
            ...
            /*  Notes: make an array/linked list of linked list based queues for each of the open sockets 
                to store the messages to be sent
                as well make an implementation of the linked list based queue
            */
        }
    }
}
